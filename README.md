...
It converts Switch/Escalation labels into binary form.
It computes both post-level and timeline-level metrics.
...
Subelement class:    multi-class per element → F1 per element, per valence, overall
...
{state}.Presence              integer (1–5)  Yes, if state present   Psychological centrality rating
{state}.{element}             object         No                      Include only elements predicted as present
{state}.{element}.subelement  integer        Yes, if element present Must be valid for that element × valence
